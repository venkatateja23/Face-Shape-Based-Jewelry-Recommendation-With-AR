def newFaceShape(imagepath):
    ...
    for i in range(1):
        if similarity < 10:
            if angle < 160:
                faceShape = faceShape + 'squared shape'
                break
            else:
                faceShape = faceShape + 'round shape'
                break
        if line3 > line1:
            if angle < 160:
                faceShape = faceShape + 'triangle shape'
                break
        if ovalsimilarity < 10:
            faceShape = faceShape + 'diamond shape'
            break
        if line4 > line2:
            if angle < 160:
                faceShape = faceShape + 'rectangular'
                break;
            else:
                faceShape = faceShape + 'oblong'
                break;
        faceShape = faceShape + "Failed to detect face shape!"
